...
/*
    Initialize Passport (authentication) to keep persistent login data (i.e in cookies)
*/
...
/*
    Flash to pop-up messages in the browser
*/
app.use(flash());
...
app.use('/app', (req, res, next) => {
    ...
    if (req.isAuthenticated()) { // if yes, continue
		res.locals.displaySettings = true;
		res.locals.displayPatient = true;
		if(req.user.role == 'doctor') {
			res.locals.displaySettings = false;
		}else if(req.user.role == 'patient') {
			res.locals.displaySettings = false;
			res.locals.displayPatient = false;
		}
        return next();
